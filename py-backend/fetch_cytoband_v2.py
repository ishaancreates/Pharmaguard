...
# Path relative to project root d:/rift/Pharmaguard
output_path = "client/src/lib/cytoBand_hg38.json"
...
try:
    ...
    sorted_chroms = sorted(cytobands.keys(), key=sort_key)
    # Filter to ensure only 1-22, X, Y are kept (remove random contigs)
    final_chroms = [c for c in sorted_chroms if c in ["chr" + str(i) for i in range(1, 23)] + ["chrX", "chrY"]]
    
    ordered_cytobands = {k: cytobands[k] for k in final_chroms}
