  ...
  const fetchMessages = async (convId) => {
    try {
      const res = await fetch(
        `http://localhost:5000/api/chat/${convId}/messages`,
      );
      const data = await res.json();
  ...
  const handleSendMessage = async (e) => {
    ...
    try {
      ...
      const res = await fetch("http://localhost:5000/api/chat/start", {
        ...
        body: JSON.stringify({
          target_user_id: activeConversation.other_user_id,
        }),
            ...
            <>
              ...
              <form
                onSubmit={async (e) => {
                  ...
                  try {
                    await fetch(
                      `http://localhost:5000/api/chat/${activeConversation.id}/messages`,
                      {
                        method: "POST",
                        headers: { "Content-Type": "application/json" },
                        body: JSON.stringify({
